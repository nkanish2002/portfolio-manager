...
function CreatePortfolioModal({ isOpen, onClose }: CreatePortfolioModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur flex items-center justify-center z-50 p-4 sm:p-6">
      <div className="bg-gray-900 border border-slate-dark rounded-none p-5 sm:p-6 w-full max-w-md mx-auto shadow-2xl" style={{ maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-black border border-slate-dark rounded-none px-3 py-3 text-white focus:outline-none focus:border-emerald-500 text-base"
              autoFocus
            />
          </div>
          <div>
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 py-3 text-slate-300 hover:text-white hover:bg-slate-dark/50 rounded-none transition-colors text-base font-medium"
            >
              Cancel
            </button>
            <button
              ...
              className="flex-1 px-4 py-3 bg-emerald-600 text-white rounded-none hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-base font-medium"
            >
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64 sm:h-96">
        <div className="animate-spin rounded-none h-10 w-10 sm:h-12 sm:w-12 border-b-2 border-emerald-400"></div>
      </div>
    );
  }
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <button
          ...
          className="w-full sm:w-auto px-4 py-3 sm:py-2 bg-emerald-600 text-white rounded-none hover:bg-emerald-700 transition-colors font-medium text-base sm:text-sm"
        >
          + New Portfolio
        </button>
      </div>
      ...
      {portfolios.length === 0 ? (
        <div className="text-center py-10 sm:py-12 bg-gray-900/50 rounded-none border border-slate-dark">
          <p className="text-slate-400 text-lg mb-4">No portfolios yet</p>
          <button
            onClick={() => setShowCreateModal(true)}
            className="w-full sm:w-auto px-6 py-3 bg-emerald-600 text-white rounded-none hover:bg-emerald-700 font-medium"
          >
